...
金額 delta，彩底顏色分級參考主播女孩重度依賴（NEEDY GIRL OVERDOSE）直播 UI 的 SC 階級：金額由低到高依序為藍、綠、黃、橘、紅，紅色為最高階：

| 金額 | 顏色 | delta |
|---|---|---|
| ¥100 | 藍 | `{aff+2, fol+60}` |
| ¥1,000 | 綠 | `{aff+5, fol+400}` |
| ¥5,000 | 黃 | `{aff+8, fol+1800}` |
| ¥10,000 | 橘 | `{aff+10, fol+2500}` |
| ¥50,000 | 紅 | `{aff+18, fol+12000}` |

送出流程：`parse(text)` → 套金額 delta + 關鍵字 delta（相加）→ 觸發她的唸稿對話。

彩底 SC 留言在互動中無法被刪除，會隨著直播持續停留在畫面上（與 §9 酸民關鍵字的「留言不可刪」同一機制）。
